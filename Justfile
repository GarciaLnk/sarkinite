...
# Build Image
build image="sarkinite-kde" tag="latest" flavor="main" rechunk="0":
    #!/usr/bin/bash
    set -eoux pipefail
    image={{ image }}
    tag={{ tag }}
...
build-rechunk image="sarkinite-kde" tag="latest" flavor="main":
    @just build {{ image }} {{ tag }} {{ flavor }} 1

# Rechunk Image
[private]
rechunk image="sarkinite-kde" tag="latest" flavor="main":
    #!/usr/bin/bash
    set -eoux pipefail
...
build-iso image="sarkinite-kde" tag="latest" flavor="main" ghcr="0":
    ...
    if [[ "{{ ghcr }}" == "1" ]]; then
        IMAGE_FULL=ghcr.io/garcialnk/"${image_name}":"${tag}"
